            ...
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="text-muted mb-0 mt-1">قم بتعديل البيانات التالية</p>
                    </div>
                        ...
                        <div class="card border-0 shadow-sm mb-4">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label fw-medium">كلمة المرور (اتركه فارغاً إذا لا تريد التغيير)</label>
                                    ...
                                    <div class="col-md-6 mb-3" id="field-governorate" style="display:none;">
                                        ...
                                        <select name="governorate_id" id="governorate_id" class="form-select">
                                            ...
                                            @foreach($governorates as $gov)
                                                <option value="{{ $gov->id }}"
                                                    {{ $user->governorate_id == $gov->id ? 'selected' : '' }}>
                                                    {{ $gov->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-6 mb-3" id="field-main-area" style="display:none;">
                                        ...
                                        <select name="main_work_area_code" id="main_work_area_code" class="form-select">
                                            <option value="">— اختر منطقة العمل الرئيسية —</option>

                                            @if($work_areas)
                                                @foreach($work_areas as $a)
                                                    <option value="{{ $a->id }}"
                                                        {{ old('main_work_area_code', $user->main_work_area_code) == $a->id ? 'selected' : '' }}>
                                                        {{ $a->name }}
                                                    </option>
                                                @endforeach
                                            @endif
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const userGovernorate = "{{ $user->governorate_id }}";
    const userCity = "{{ $user->city_id }}";
    const userWorkArea = "{{ $user->main_work_area_code }}";
    ...
    function showRolePermissions(ids) {
        ...
        roleDiv.innerHTML = ids.map(id =>
            `<span class="badge bg-success me-1 mb-1">${permissionNames[id]}</span>`
        ).join("");
    }

    function applyRoleBehavior(roleName) {
    ...
    function loadCitiesAndAreas(govId, selectedCity = null, selectedArea = null) {
...
fetch(`/get-work-areas-users/${govId}?user_id={{ $user->id }}`)
            ...
            .then(data => {
                ...
                let currentSelected = selectedArea ?? areaSelect.value;

                areaSelect.innerHTML = '<option value="">— اختر منطقة العمل الرئيسية —</option>';

                data.forEach(a => {
                    areaSelect.innerHTML += `<option value="${a.id}" ${currentSelected == a.id ? 'selected' : ''}>${a.name}</option>`;
                });

                if (userRoleName === "survey_supervisor") {
                    fieldArea.style.display = "block";
                }
            });
    }

    applyRoleBehavior(userRoleName);

    const currentRoleId = roleSelect.value;
    showRolePermissions(rolesData[currentRoleId] ?? []);

    perms.forEach(p => {
        if (rolesData[currentRoleId]?.includes(parseInt(p.value))) {
            p.checked = true;
        }
    });

    if (userGovernorate) {
        loadCitiesAndAreas(userGovernorate, userCity, userWorkArea);
    }
    ...
    governorateSelect.addEventListener("change", () => {
        ...
        let currentAreaSelected = areaSelect.value;
        ...
        loadCitiesAndAreas(govId, null, currentAreaSelected);
    });
